...
function changeAdult(n) { let na = adults+n; if(na<1||na>MAX_ADULTS||na+children>MAX_TOTAL)return; adults=na; document.getElementById("adultCount").innerText=adults; updateButtons(); calculate();}
function changeChild(n) { let nc = children+n; if(nc<0||adults+nc>MAX_TOTAL)return; children=nc; document.getElementById("childCount").innerText=children; updateButtons(); calculate();}
function calculate(){
...
calendarInput.addEventListener("change",()=>{let dates=calendarInput._flatpickr.selectedDates;if(dates.length===2){startDate=dates[0];endDate=dates[1];calculate();}});
...
document.getElementById("checkoutButton").addEventListener("click",()=>{
    ...
    summary.innerHTML=`Du <b>${startDate.toLocaleDateString("fr-FR")}</b> au <b>${endDate.toLocaleDateString("fr-FR")}</b><br><b>${adults}</b> adulte(s), <b>${children}</b> enfant(s)<br>Total estimé : <b>${calculate().toFixed(2)} €</b>`;
...
document.getElementById("whatsappFloat").addEventListener("click",()=>{
    ...
    if(startDate&&endDate){msg=`Bonjour, je souhaite réserver Villa CABOUA du ${startDate.toLocaleDateString("fr-FR")} au ${
